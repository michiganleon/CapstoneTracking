...
class ReusltQueue(PriorityQueue):
	...
	def put(self, item):
		if (item[2] > self.lowesrscore):
			PriorityQueue.put(self, (item[2], self.counter, item))
			self.counter += 1
			if(self.counter == 1):
				self.lowesrscore = item[2]
			...
			if(self.counter > self.upperlimmit):
				...
				self.lowesrscore = new_small[2]
				PriorityQueue.put(self, (new_small[2], self.counter, new_small))
